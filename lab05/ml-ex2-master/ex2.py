...
def ex2():
    ...
    y = data[:, 2]
    print()
    print(f"0-s osztály: {np.sum(y==0)}")
    print(f"1-s osztály: {np.sum(y==1)}")
    ...
    print(
        "For a student with scores 45 and 85, we predict an admission probability of {:0.3f}".format(
            prob
        )
    )
...
# hf bonyolitani a modellt ugy,hog yne egyenes legyen a kep hanem magasabb rendu polinom
